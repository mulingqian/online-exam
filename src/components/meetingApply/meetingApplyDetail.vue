    ...
    <div class="containers-common">
      ...
      <div  v-if="r">
        <div class="detail">
          ...
          <div class="deSecond">
            <span class="bt"><span class="btword">会议名称</span><span class="ggbt">{{r.HYMC}}</span></span>
            <span class="bt"><span class="btword">会议主题</span>{{r.HYZT}}</span>
            <span class="bt"><span class="btword">座位席卡</span>{{r.ZWXK}}</span>
            <span class="bt"><span class="btword">开始时间</span>{{r.KSSJ}}</span>
            <span class="bt"><span class="btword">结束时间</span>{{r.JSSJ}}</span>
            <span class="bt"><span class="btword">会后用餐</span>{{r.HHYC}}</span>
            <span class="bt"><span class="btword">会议性质</span>{{r.HYXZ}}</span>
            <span class="bt"><span class="btword">紧急度</span>{{r.JJD}}</span>
            <span class="bt"><span class="btword">重要性</span>{{r.ZYX}}</span>
            <span class="bt"><span class="btword">会议室</span>{{r.RoomName}}</span>
            <span class="bt"><span class="btword">会议人数</span>{{r.HYRS}}</span>
            <span class="bt"><span class="btword">登记时间</span>{{r.ZYX}}</span>
            <span class="bt" v-if="r.SubmitTime !== ''"><span class="btword">提交时间</span>{{r.SubmitTime}}</span>
            <span class="bt"><span class="btword">会议设备</span>{{r.RoomDevices}}</span>
            <span class="bt"><span class="btword">内部出席人员</span>{{r.NBCXRY}}</span>
            <span class="bt"><span class="btword">外部出席人员</span>{{r.WBCXRY}}</span>
            <span class="bt"><span class="btword">备注</span>{{r.Remark}}</span>
          </div>
        </div>
        ...
        <div style="padding: 10px 15px;font-size: 16px;background-color: #fff;">
          ...
          <div class="nr">
            <span>内容</span>
            <div class="dContents" v-html="r.ZYNR"></div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      goSelectMultiTeacher () {
        // 保存审批意见
        sessionStorage.setItem("meetingApplyDetail-verifyContent", this.verifyContent);
        // 之前的选择的教师信息
        if (this.selectMultiEmployeeUserIdLst !== '') {
          sessionStorage.setItem("selectMultiEmployeeNameLst_meetingApplyDetail", this.selectMultiEmployeeNameLst); // 存选择教师的姓名
          sessionStorage.setItem("selectMultiEmployeeUserIdLst_meetingApplyDetail", this.selectMultiEmployeeUserIdLst); // 存选择教师的UserId
        }
        this.$router.push({path: '/selectMultiTeacher', query: {page: 'meetingApplyDetail', type: this.$route.query.type, id: this.$route.query.id, aId: this.$route.query.aId, rId: this.$route.query.rId}});
      },
      //  撤回操作type=0'新闻发布申请'列表（所有审批人尚未进行过审批操作时，提交人进行的撤回）RevokeBizData
      async revokeBizData () {
        let name = this.QD.getLocalUserinfo().stuObj.XM;
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.RevokeBizData', removerName: name, cellId: 'OA_FLOW_SQ_HYSQ', bizId: this.bizId});
      ...
      async getCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_HYSQ', auditedOnly: false});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      // 判断是从type=0'会议申请'列表，还是type=1'会议申请审批'列表的详情
      getType () {
        this.type = parseInt(this.$route.query.type);
        this.bizId = parseInt(this.$route.query.id);
        if (this.type === 0) {
          this.cellId = 'OA_FLOW_SQ_HYSQ';
        } else {
          this.cellId = 'OA_FLOW_SQ_HYSQ';
        }
      },
      ...
      async isGetCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_HYSQ', auditedOnly: true});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      getSelectMultiTeacherData () {
        if (sessionStorage.getItem("selectMultiEmployeeNameLst_meetingApplyDetail") !== null && sessionStorage.getItem("selectMultiEmployeeUserIdLst_meetingApplyDetail") !== null){
          let name = sessionStorage.getItem("selectMultiEmployeeNameLst_meetingApplyDetail").split(",");
          let userId = sessionStorage.getItem("selectMultiEmployeeUserIdLst_meetingApplyDetail").split(",");
          ...
          console.log("xm is", this.selectMultiEmployeeNameLst);
          sessionStorage.removeItem("selectMultiEmployeeNameLst_meetingApplyDetail");
          sessionStorage.removeItem("selectMultiEmployeeUserIdLst_meetingApplyDetail");
        } else {
          this.selectMultiEmployeeNameLst = ['选择'];
        }
        // 审批意见
        if (sessionStorage.getItem("meetingApplyDetail-verifyContent") !== null) {
          this.verifyContent = sessionStorage.getItem("meetingApplyDetail-verifyContent");
          sessionStorage.removeItem("meetingApplyDetail-verifyContent");
        }
      }
    },
    created () {
...
<style>
  .meetingApplyDetail .verify .weui-cells__title{
    margin-top: 0;
    margin-bottom: 0;
    padding-top: 0.3em;
  ...
  .meetingApplyDetail .verify .weui-cell_access .weui-cell__ft {
    display: inline-block;
    width: 60%;
    text-align: right;
    overflow: hidden;
