<template>
  <div class="meetingApply">
    ...
    <tab>
      <tab-item :selected="showLst" @on-item-click="adjustApplyOrVerify(0)">会议申请</tab-item>
      <tab-item v-if="canAudit" :selected="!showLst" @on-item-click="adjustApplyOrVerify(1)">会议申请审批</tab-item>
    </tab>
    <!--会议申请-->
    <div v-show="showLst" class="applyShowLst">
      <ex-scroller ref="scrollerApplyPageIdxOne" v-model="meetingApplyData" :onGetList="getMeetingApplyData" height="-90">
        <div>
          <group v-for="(n, index) in meetingApplyData" :key="index">
            <cell is-link @click.native="goMeetingDetailPage(n.HYID,0,0)">
              <span slot="after-title" class="vux-label">
                ...
                <div class="firstLine">会议名称：{{n.HYMC}}</div>
                <div class="secondLine">会议主题：{{n.HYZT}}</div>
                <div class="thirdLine">开始时间：{{n.KSSJ}}</div>
                <div class="forthLine">结束时间：{{n.JSSJ}}</div>
    ...
    <div v-if="canAudit && !showLst" class="verifyShowLst">
      <ex-scroller ref="scrollerAuditPageIdxOne" v-model="meetingVerifyData" :onGetList="getMeetingVerifyData" height="-90">
        <div>
          <group v-for="(n, index) in meetingVerifyData" :key="index">
            <cell is-link @click.native="goMeetingDetailPage(n.HYID,n.AppId,n.REID)">
              <span slot="after-title" class="vux-label">
                <span class="fileImportAndUrgent">
                  <span class="fileImport">
                    <span :class="`school-wx-icon icon-importFile ${n.ZYX === '非常重要' ? 'veryImporRed': (n.ZYX === '重要'? 'imporYellow': '')}`" slot="icon" style="font-size: 35px;">
...
<script>
  ...
  export default {
    ...
    methods: {
      // 判断是‘会议申请’列表，还是 ‘会议申请’列表
      adjustApplyOrVerify (type) {
        this.showType = type;
        this.showLst = this.showType === 0;
      },
      // 到‘会议详情’页面,showType=0代表'会议申请'列表，showType =1代表'会议申请'列表
      goMeetingDetailPage (id, aId, rid) {
        // 把'会议申请'的type 存sessionStorage,从'会议申请'页面 可以返回'会议申请'列表
        sessionStorage.setItem("meetingShowType", this.showType);
        this.$router.push({path: '/meetingApply/meetingApplyDetail', query: {type: this.showType, id: id, aId: aId, rId: rid}});
      },
      // 到‘会议申请’页面
      /*goNewsPublishApply () {
      ...
      // 获取‘会议申请’列表
      async getMeetingApplyData (index, size) {
        // 判断显示哪个列表
        if (sessionStorage.getItem("meetingShowType") !== null) {
          this.showType = parseInt(sessionStorage.getItem("meetingShowType"));
          this.showLst = this.showType === 0;
          // 清除存'会议申请'列表 meetingShowType
          sessionStorage.removeItem("meetingShowType");
        }
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetUserSubmitFlowData', cellId: 'OA_FLOW_SQ_HYSQ', pageIndex: index, pageSize: size, conditionList: ''});
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
        console.log("meetingApplyData:", d.data);
        return d.data;
      },
      // 获取‘会议申请审批’列表
      async getMeetingVerifyData (index, size) {
        if (this.canAudit === true) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetUserAuditFlowData', cellId: 'OA_FLOW_SQ_HYSQ', pageIndex: index, pageSize: size, conditionList: ''});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
          console.log("meetingSPData:", d.data);
          return d.data;
        }
      },
      // 读取用户可申请和审批的流程菜单
      async getUserMenus () {
        ...
        for (let i=0; i<data.length; i++) {
          ...
          if (data[i].applyCellId === "OA_FLOW_SQ_HYSQ"){
    ...
    data () {
      return {
        ...
        meetingApplyData: [], // 会议申请列表
        meetingVerifyData: [], // 会议申请审批列表
...
<style>
  .meetingApply .verifyShowLst .weui-cells,
  .meetingApply .applyShowLst .weui-cells {
    position: relative;
    left: 2%;
    width: 96%;
    margin-top: 18px;
