<template>
  ...
  <div class="schoolCard" :class="[bgColor ? 'greenColor': 'redColor']">
    ...
    <!--模态框 start-->
    <div v-transfer-dom>
      <popup v-model="showQrCode" height="270px;" is-transparent>
        <div style="" class="popup-box">
          <p>请扫描{{popupTitle}}
            <span class="school-wx-icon" @click="showQrCode=false">&#xe627;</span>
          </p>
          <div class="barcode-box" v-if="popupTitle === '条形码'">
            <img id="barcode1" ref="barcode"/>
          </div>
          <qrcode class="qr-code" :value="qrCodeInfo" v-else></qrcode>
        </div>
      </popup>
    </div>
    <!--模态框 end-->
...
<script>
  ...
  import clock from "./componens/time.vue";
