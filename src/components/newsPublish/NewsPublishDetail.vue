    ...
    <div class="containers-common">
      ...
      <div v-if="r">
        <div class="detail">
          ...
          <div class="deSecond">
            <span class="bt"><span>标题</span>{{r.XWBT}}</span>
            <span class="bt"><span>发送范围</span>{{r.FSFW}}</span>
            <span class="bt"><span>是否置顶</span>{{rOnTop}}</span>
            <span class="bt"><span>截止时间</span>{{r.JZRQ}}</span>
            <span class="bt"><span>紧急度</span>{{r.JJD}}</span>
            <span class="bt"><span>重要性</span>{{r.ZYX}}</span>
            <span class="bt"><span>是否已发布</span>{{rPublished}}</span>
            <span class="bt"><span>自动记录已阅</span>{{rYYJL}}</span>
            <span class="bt"><span>要求阅读回复</span>{{rYYQR}}</span>
            <span class="bt"><span>提交时间</span>{{r.SubmitTime}}</span>
          </div>
        </div>
        ...
        <div style="padding: 10px 15px;font-size: 16px;background-color: #fff;">
          ...
          <div v-if="imgAtts.length > 0 || fileAtts.length > 0" class="fj">
            ...
            <div v-for="(a,idx) in fileAtts" :key="idx" style="display: inline-block;padding: 25px;">
              ...
              <a :href="QD.OAHOST + a.FilePath" :download="a.FileName" target="_blank" style="color: #008000;font-size: 14px;">{{a.FileName}}</a>
            </div>
          </div>
          <!--内容-->
          <div class="nr">
            <span>内容</span>
            <div class="dContents" v-html="r.Contents"></div>
          </div>
        </div>
        ...
        <div v-if="type === 0 && verifyedResults.length === 0 && r.States !== '0' && this.r.States !== '9'" style="height: 70px;background-color: #f2f2f2;"></div>
      ...
      <!--申请列表中可以撤销-->
      <footer v-if="r && type === 0 && verifyedResults.length === 0 && r.States !== '0' && this.r.States !== '9'" class="cancel">
        <a @click="revokeBizData()">撤回</a>
      </footer>

      <!--审批 States=3是‘通过’；States=2是‘审核中’-->
  ...
  export default {
    ...
    methods: {
      ...
      goSelectMultiTeacher () {
          // 存审批意见
        sessionStorage.setItem("newsPublish-verifyContent", this.verifyContent);
        // 之前的选择的教师信息
        console.log("");
        if (this.selectMultiEmployeeUserIdLst !== '') {
         // 存选择教师的姓名
          sessionStorage.setItem("selectMultiEmployeeNameLst_newsPublish", this.selectMultiEmployeeNameLst);
          // 存选择教师的UserId
          sessionStorage.setItem("selectMultiEmployeeUserIdLst_newsPublish", this.selectMultiEmployeeUserIdLst);
        }
        this.$router.push({path: '/selectMultiTeacher', query: {page: 'newsPublishDetail', type: this.$route.query.type, id: this.$route.query.id, aId: this.$route.query.aId, rId: this.$route.query.rId}});
      },
        // 审批‘通过’或 ‘拒绝’
      async doAudit (pass) {
        let appointUids = '';
      ...
      //  撤回操作type=0'新闻发布申请'列表（所有审批人尚未进行过审批操作时，提交人进行的撤回）RevokeBizData
      async revokeBizData () {
        let name = this.QD.getLocalUserinfo().stuObj.XM;
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.RevokeBizData', removerName: name, cellId: 'OA_FLOW_SQ_XWFB', bizId: this.bizId});
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
        ...
        if (d.status === 'success') {
          this.getGetFlowInfo();
          // 根据流程申请业务模块的CellID和键值读取审批结果（审批步骤-审批人-回复内容等）auditedOnly = true仅查看已经审批的结果
      ...
      getSelectMultiTeacherData () {
        ...
        if (sessionStorage.getItem("selectMultiEmployeeNameLst_newsPublish") !== null && sessionStorage.getItem("selectMultiEmployeeUserIdLst_newsPublish") !== null){
          let name = sessionStorage.getItem("selectMultiEmployeeNameLst_newsPublish").split(",");
          let userId = sessionStorage.getItem("selectMultiEmployeeUserIdLst_newsPublish").split(",");
          this.selectMultiEmployeeNameLst = name.join();
          this.selectMultiEmployeeUserIdLst = userId.join();
          console.log("this.selectMultiEmployeeNameLst", typeof(this.selectMultiEmployeeNameLst));
          console.log("selectMultiEmployeeUserIdLst", typeof(this.selectMultiEmployeeUserIdLst));
          console.log(this.selectMultiEmployeeUserIdLst);
          console.log("this.selectMultiEmployeeUserIdLst");
          console.log("this.selectMultiEmployeeNameLst", this.selectMultiEmployeeNameLst);
          sessionStorage.removeItem("selectMultiEmployeeNameLst_newsPublish");
          sessionStorage.removeItem("selectMultiEmployeeUserIdLst_newsPublish");
        } else {
          this.selectMultiEmployeeNameLst = ['选择'];
        }
        if (sessionStorage.getItem("newsPublish-verifyContent") !== null) {
          this.verifyContent = sessionStorage.getItem("newsPublish-verifyContent");
          sessionStorage.removeItem("newsPublish-verifyContent");
        }
      },
      // 读取流程实体数据
      async getGetFlowInfo () {
        // 判断是从'新闻发布申请'列表，还是'新闻发布审批'列表的详情
        this.getType();
        ...
        this.rOnTop = this.r.OnTop !== 'False' ? '是' : '否';
        this.rPublished = this.r.Published !== 'False' ? '是' : '否';
        this.rYYJL = this.r.YYJL === 'True' ? '是' : '否';
        this.rYYQR = this.r.YYQR !== 'False' ? '是' : '否';
      ...
      async getCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_XWFB', auditedOnly: false});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      async isGetCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_XWFB', auditedOnly: true});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      // 判断是从type=0'新闻发布申请'列表，还是type=1'新闻发布审批'列表的详情
      getType () {
        this.type = parseInt(this.$route.query.type);
        this.bizId = parseInt(this.$route.query.id);
        if (this.type === 0) {
          this.cellId = 'OA_FLOW_SQ_XWFB';
        } else {
          this.cellId = 'OA_FLOW_SP_XWFB';
        }
      },
      // 读取审批人对当前流程的审批结果及各种状态
    ...
    data () {
      return {
        ...
        selectMultiEmployeeUserIdLst: '', // 选择教师UserId字符串
        rStates: '', // 流程状态值
        rOnTop: '', //是否置顶
        rPublished: '', // 是否已发布
        rYYJL: '', //自动记录已阅
...
<style>
  .newsPublishDetail .verify .weui-cells__title{
    margin-top: 0;
    margin-bottom: 0;
    padding-top: 0.3em;
  ...
  .newsPublishDetail .verify .weui-cell_access .weui-cell__ft {
    display: inline-block;
    width: 60%;
    text-align: right;
    overflow: hidden;
