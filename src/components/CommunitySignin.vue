  ...
  <div class="page-detail">
    ...
    <scroller lock-x height="-102" @on-scroll-bottom="onScroll" ref="scroller">
      <div>
        ...
        <group title="社团签到记录" v-if="!!objInfo" >
          <cell  v-for="(obj, index) in students" :key="index"  :border-intent="false">
            ...
            <span slot="value" class="stumess">
              <span v-on:click="doSign(objInfo.ID, obj.XH)">
                <x-button mini plain v-bind:class="obj.sign ? 'btn-qd' : 'btn-in'">
                  <span class="demo-icon">&#xe718;</span>签到
                </x-button>
              </span>
              <span v-on:click="doSignout(objInfo.ID, obj.XH)">
                <x-button mini plain v-bind:class="obj.signout ? 'btn-qt' : 'btn-in'">
                  <span class="demo-icon" >&#xe60f;</span>签退
                </x-button>
              </span>
            </span>
          </cell>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 社团成员签退
      async doSignout (ParentId, XH) {
        let signout;
        for (let i = 0; i < this.students.length; i++) {
          if (XH === this.students[i].XH) {
            if (this.students[i].signout === undefined) {
              this.students[i].signout = true;
            } else {
              this.students[i].signout = !this.students[i].signout;
            }
            signout = this.students[i].signout;
            this.$set(this.students, i, this.students[i]);
          }
        }
        let Details = '[{"XH":"' + XH + '","ParentId":"' + ParentId + '","SignOut": ' + signout + ',"SignOutDate":"' + this.datatimes + '","SignOutRemark":"签退"}]';
        let d = await this.QD.getData('/School/StuClient/UpdateClubAttendance', {entityJson: Details});
      ...
      async doSign (ParentId, XH) {
        ...
        let Details = '[{"XH":"' + XH + '","ParentId":"' + ParentId + '","Sign": ' + sign + ',"SignDate":"' + this.datatimes + '","SignRemark":"签到","SignOut":false}]';
        let d = await this.QD.getData('/School/StuClient/InsertClubAttendance', {entityJson: Details});
        console.log(d.status);
      },
      async getDetail () {
        ...
        let w = await this.QD.getData('/School/StuClient/StudentClubAttendanceData', { ParentId: parseInt(d.ID), CreateDate: this.datatimes })
        console.log(w);
        for (let j = 0; j < w.length; j++) {
          for (let i = 0; i < this.students.length; i++) {
            if (this.students[i].XH === w[j].XH) {
              this.students[i].sign = w[j].Sign;
              this.students[i].signout = w[j].Signout;
            }
          }
        }
        this.loading = false
