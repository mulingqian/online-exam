  ...
  <div class="selectTeacher">
    <div class="allDepartmentAndEmployee">
      ...
      <div class="employeeLst">
        ...
        <scroller lock-x :scrollbar-y="true" height="-95" style="background-color: #f2f2f2;">
          <div>
            <a v-for="(employee, eIdx) in employeeData" :key="eIdx" @click="getSelectEmployData(employee.XM,employee.ZGH, employee.UserId)">{{employee.XM}}</a>
          </div>
        </scroller>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 选择教师后做处理
      getSelectEmployData (name, zgh, userId) {
        if (this.$route.query.page === 'fieldSingleSTea') {
          // 组件 fieldDingleSTea.vue
          sessionStorage.setItem("selectSingleEmployeeNameLst_fieldSingleSTea", name); // 存选择教师的姓名
          sessionStorage.setItem("selectSingleEmployeeUserIdLst_fieldSingleSTea", userId); // 存选择教师的职工号
          this.$router.replace({path: '/userDefinded/officeTwo'}); //返回到公文流转信息页面
        } else {
          sessionStorage.setItem("selectEmployeeName", name); // 存选择教师的姓名
          sessionStorage.setItem("selectEmployeeZGH", zgh); // 存选择教师的职工号
          this.$router.replace({path: '/receiptManagement/acceptFileDetail', query: {id: this.$route.query.id}}); //返回到公文流转信息页面
        }
      }
    },
    created (){
...
<style>
  ...
  .selectTeacher .allDepartment .xs-container{ margin-top: 0; background-color: #ffffff;}
</style>
