  ...
  <div class="infirmSickReg">
    ...
    <group style="margin-bottom: 60px;">
      ...
      <cell>
        ...
        <span class="rg-cell">{{leaveApplyType}}</span>
      </cell>
      <cell is-link>
        <span slot="after-title" class="vux-label">
         <span>是否需要离校</span>
    ...
    <submit-button :title="submitTitle" @click-button="addSubmit"></submit-button>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async addSubmit () {
        ...
        if (d > 0){
          let sDay = this.startTime.substr(0, 10);
          let eDay = this.endTime.substr(0, 10);
          let endD = eDay.replace(/-/g, '/');
          let end = new Date(endD);
          let lstDate = [];
          while (sDay.length > 0) {
            lstDate.push(sDay);
            sDay = this.QD.addDate(sDay, 1);
            let startDate = sDay.replace(/-/g, '/');
            let begin = new Date(startDate);
            if (end < begin) {
              break;
            }
          }
          console.log('lstDate', lstDate);
          for (let i in lstDate) {
            /*let stulst = [{Category: 32, StuNum: 1, StuRemark: this.seleStuXH}];
            let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'InsertStuentAttendanceData',
              JXBID: 8206,
              Nodes: "3",
              TeachDay: lstDate[i],
              ResultList: JSON.stringify(stulst)
            });*/
            this.getStuSmstClassData(lstDate[i]);
          }
          this.$router.go(-1);
        } else {
          this.QD.alert('', '登记失败！');
      ...
      async getStuSmstClassData (dy) {
        this.loading = true;
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'GetStuSmstClassData', xh: this.seleStuXH, date: dy});
        this.loading = false;
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
        let listCategory = [{
          Category: 32,
          StuNum: 1,
          StuRemark: this.seleStuXH
        }];
        if (d.length > 0) {
          for (let i in d) {
            this.saveKaoQin(d[i].ClaId, d[i].NodeIndex, listCategory, dy);
          }
        }
      ...
      // 将数据同步到课堂日志
      async saveKaoQin (jxbid, node, lst, dy) {
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'InsertStuentAttendanceData',
          JXBID: jxbid,
          Nodes: node,
          TeachDay: dy,
          ResultList: JSON.stringify(lst)
        });
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
        if (d !== null) {
          if (parseInt(d) <= 0) {
            if (d === -1) {
              ...
              // this.QD.alert('', '当前考勤记录已添加，无法再次进行添加！');
            } else {
      ...
      getPageData () {
        console.log("QD.otherPageData", this.QD.otherPageData);
        ...
        console.log('seleStuXH seleStuName', this.seleStuXH, this.seleStuName );
      },
      getCurrPGData () {
        this.getPageData();
        // 取当前页面之前存的有关学生请假的数据
...
<style>
  .infirmSickReg .weui-cells{ margin-top:0;}
  .infirmSickReg .weui-cell .vux-label{
    color: #333333;
  }
  .infirmSickReg .weui-cell .weui-cell__ft{
    color: #333333;
  }
  .infirmSickReg .weui-cells .vux-datetime .vux-datetime-value .vux-cell-value{
    color: #666666;
  }
  .infirmSickReg .weui-cells .vux-datetime div p{
    color: #333333;
  }
  /*状态值面板*/
  .infirmSickReg .weui-actionsheet__menu .weui-actionsheet__cell{
    font-size: 16px;
    color: #333333;
  }
  .infirmSickReg .weui-actionsheet__action .weui-actionsheet__cell{
    position: relative;
    padding: 10px 0;
    text-align: center;
    font-size: 16px;
  ...
  .infirmSickReg .vux-x-textarea.weui-cell .weui-cell__bd textarea{
    font-size: 14px;
    color: #333333;
  }
</style>
