      ...
      <div class="allDepartment">
        ...
        <scroller lock-x :scrollbar-y="false" height="-95" style="background-color: #fff;">
          <div>
            ...
            <a v-if="departmentData.length !== 0" :class="{'clickBgColor':dIdx === isDepartActive}" v-for="(depart, dIdx) in departmentData" v-on:click="getEmployeeLst(depart.KSID,dIdx)" :key="dIdx">{{depart.KSMC}}</a>
          </div>
        </scroller>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      async getAllDepartment() {
        ...
        this.getEmployeeData(this.departmentData[0].KSID); // 获取第一列部门中的教师信息
      },
      // 根据部门代码查询教师信息
      async getEmployeeData(ksid) {
        let search = [{"FieldName": "KSID", "Value": parseInt(ksid), "NeedQuotes": "true", "Operator": "Equal"}];
        this.loading = true;
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'ClassMasterData', search: JSON.stringify(search)});
        this.loading = false;
        if (d.status && d.status === "error") {
          this.QD.alert(`获取教师信息失败:${d.msg}`);
          return;
        }
        this.employeeData = d.List;
      ...
      // 根据不同部门的代码获取教师信息
      getEmployeeLst (ksid, idx) {
        this.getEmployeeData(ksid);
      ...
      chooseEmployee (idx) {
        ...
        if (this.employeeData[idx].selectedEmploy === true) {
          ...
          this.selectedEMID.push(this.employeeData[idx].EMID);
        } else {
          // 去掉重复的
      ...
      getSelectEmployeeData () {
        if (this.QD.otherPageData) {
          ...
          this.selectedEMID = this.QD.otherPageData.emId;
      ...
      // 保存本页数据
      saveCurrData () {
        this.QD.otherPageData = {nameLst: this.selectedEmployeeNameLst, zghLst: this.selectedEmployeeZGHLst, emId: this.selectedEMID};
      }
    },
    beforeDestroy () {
    },
