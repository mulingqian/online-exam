<template>
  <div class="receiptFileManage">
    <div v-transfer-dom>
      <loading v-model="loading"></loading>
    </div>
    <tab>
      <tab-item :selected="showLst" @on-item-click="adjustApplyOrVerify(0)">收文管理申请</tab-item>
      <tab-item v-if="canAudit" :selected="!showLst" @on-item-click="adjustApplyOrVerify(1)">收文管理审批</tab-item>
    </tab>
    <!--收文管理申请-->
    <div v-show="showLst" class="applyShowLst">
      <ex-scroller ref="scrollerApplyPageIdxOne" v-model="fileManageApplyData" :onGetList="getReceiptFileManageApplyData" height="-90">
        <div>
          <group v-for="(n, index) in fileManageApplyData" :key="index">
            <cell is-link @click.native="goFileManageDetailPage(n.SWID,0,0)">
              <span slot="after-title" class="vux-label">
                ...
                <div class="firstLine">标题：{{n.SWBT}}</div>
                <div class="secondLine">文号：{{n.SWWH}}</div>
                <div class="thirdLine">来文日期：{{n.CreateTime}}</div>
    ...
    <div v-if="canAudit && !showLst" class="verifyShowLst">
      <ex-scroller ref="scrollerAuditPageIdxOne" v-model="fileManageVerifyData" :onGetList="getFileManageVerifyData" height="-90">
        <div>
          <group v-for="(n, index) in fileManageVerifyData" :key="index">
            <cell is-link @click.native="goFileManageDetailPage(n.SWID,n.AppId,n.REID)">
              <span slot="after-title" class="vux-label">
                <span class="fileImportAndUrgent">
                  <span class="fileImport">
                    <span :class="`school-wx-icon icon-importFile ${n.ZYX === '非常重要' ? 'veryImporRed': (n.ZYX === '重要'? 'imporYellow': '')}`" slot="icon" style="font-size: 35px;">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 判断是‘收文管理申请’列表，还是 ‘收文管理审批’列表
      adjustApplyOrVerify (type) {
        this.showType = type;
        this.showLst = this.showType === 0;
      },
      // 到‘收文管理申请’页面,showType=0代表'收文管理申请'列表，showType =1代表'收文管理申请'列表
      goFileManageDetailPage (id, aId, rid) {
        // 把'收文管理申请'的type 存sessionStorage,从'收文管理申请'页面 可以返回'收文管理申请'列表
        sessionStorage.setItem("receiptFileShowType", this.showType);
        this.$router.push({path: '/receiptFile/receiptFileManageDetail', query: {type: this.showType, id: id, aId: aId, rId: rid}});
      },
      // 到‘收文管理申请’页面
      /*goNewsPublishApply () {
      ...
      // 获取‘收文管理申请’列表
      async getReceiptFileManageApplyData (index, size) {
        // 判断显示哪个列表
        if (sessionStorage.getItem("receiptFileShowType") !== null) {
          this.showType = parseInt(sessionStorage.getItem("receiptFileShowType"));
          this.showLst = this.showType === 0;
          // 清除存'收文管理申请'列表 receiptFileShowType
          sessionStorage.removeItem("receiptFileShowType");
        }
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetUserSubmitFlowData', cellId: 'OA_FLOW_SQ_SWGL', pageIndex: index, pageSize: size, conditionList: ''});
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
        console.log("receiptFileMgSQData:", d.data);
        return d.data;
      },
      // 获取‘收文管理审批’列表
      async getFileManageVerifyData (index, size) {
        if (this.canAudit === true) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetUserAuditFlowData', cellId: 'OA_FLOW_SP_SWGL', pageIndex: index, pageSize: size, conditionList: ''});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
          console.log("receiptFileMgSPData:", d.data);
          return d.data;
        }
      },
      // 读取用户可申请和审批的流程菜单
      async getUserMenus () {
        ...
        for (let i=0; i<data.length; i++) {
          ...
          if (data[i].applyCellId === "OA_FLOW_SQ_SWGL"){
    ...
    data () {
      return {
        ...
        showType: 0, // 默认0是‘收文管理申请’列表，1是 ‘收文管理审批’列表
        fileManageApplyData: [], // 收文管理申请申请列表
        fileManageVerifyData: [], // 收文管理审批列表
...
<style>
  .receiptFileManage .verifyShowLst .weui-cells,
  .receiptFileManage .applyShowLst .weui-cells {
    position: relative;
    left: 2%;
    width: 96%;
    margin-top: 18px;
