<template>
  <div class="receiptFileManageDetail">
    <div class="containers-common">
      ...
      <div v-if="r">
        <div class="detail">
          ...
          <div class="deSecond">
            <span class="bt"><span class="btword">标题</span><span class="ggbt">{{r.SWBT}}</span></span>
            <span class="bt"><span class="btword">文号</span>{{r.SWWH}}</span>
  ...
  export default {
    ...
    methods: {
      ...
      goSelectMultiTeacher () {
        // 保存审批意见
        sessionStorage.setItem("receiptFileManage-verifyContent", this.verifyContent);
        // 之前的选择的教师信息
        if (this.selectMultiEmployeeUserIdLst !== '') {
          sessionStorage.setItem("selectMultiEmployeeNameLst_receiptFileManage", this.selectMultiEmployeeNameLst); // 存选择教师的姓名
          sessionStorage.setItem("selectMultiEmployeeUserIdLst_receiptFileManage", this.selectMultiEmployeeUserIdLst); // 存选择教师的UserId
        }
        this.$router.push({path: '/selectMultiTeacher', query: {page: 'receiptFileManageDetail', type: this.$route.query.type, id: this.$route.query.id, aId: this.$route.query.aId, rId: this.$route.query.rId}});
      },
      // 审批‘通过’或 ‘拒绝’
      async doAudit (pass) {
        let appointUids = '';
        if (this.flowAuditInfo.allowAppoint === false) {
          ...
          if (this.selectMultiEmployeeUserIdLst.length === '') {
            ...
            console.log(appointUids);
            console.log(typeof(appointUids));
          }
        }
        this.loading = true;
      ...
      async revokeBizData () {
        ...
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.RevokeBizData', removerName: name, cellId: 'OA_FLOW_SQ_SWGL', bizId: this.bizId});
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
      ...
      async getGetFlowInfo () {
        // 判断是从type=0'收文管理申请'列表，还是type=1'收文管理审批'列表的详情
        this.getType();
      ...
      async getCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_SWGL', auditedOnly: false});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      getType () {
        ...
        if (this.type === 0) {
          this.cellId = 'OA_FLOW_SQ_SWGL';
        } else {
          this.cellId = 'OA_FLOW_SP_SWGL';
        }
      },
      // 根据流程申请业务模块的CellID和键值读取审批结果（审批步骤-审批人-回复内容等）auditedOnly = true仅查看已经审批的结果---用于判断是否显示‘撤回’
      async isGetCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_SWGL', auditedOnly: true});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      getSelectMultiTeacherData () {
        if (sessionStorage.getItem("selectMultiEmployeeNameLst_receiptFileManage") !== null && sessionStorage.getItem("selectMultiEmployeeUserIdLst_receiptFileManage") !== null){
          let name = sessionStorage.getItem("selectMultiEmployeeNameLst_receiptFileManage").split(",");
          let userId = sessionStorage.getItem("selectMultiEmployeeUserIdLst_receiptFileManage").split(",");
          ...
          sessionStorage.removeItem("selectMultiEmployeeNameLst_receiptFileManage");
          sessionStorage.removeItem("selectMultiEmployeeUserIdLst_receiptFileManage");
        } else {
          this.selectMultiEmployeeNameLst = ['选择'];
        }
        // 审批意见
        if (sessionStorage.getItem("receiptFileManage-verifyContent") !== null) {
          this.verifyContent = sessionStorage.getItem("receiptFileManage-verifyContent");
          sessionStorage.removeItem("receiptFileManage-verifyContent");
        }
      }
    },
    created () {
...
<style>
  .receiptFileManageDetail .verify .weui-cells__title{
    margin-top: 0;
    margin-bottom: 0;
    padding-top: 0.3em;
  ...
  .receiptFileManageDetail .verify .weui-cell_access .weui-cell__ft {
    display: inline-block;
    width: 60%;
    text-align: right;
    overflow: hidden;
