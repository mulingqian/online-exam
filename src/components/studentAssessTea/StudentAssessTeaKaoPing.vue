<template>
  <div class="studentAssessKaoPing">
    <div v-transfer-dom>
        ...
        <div v-for="(item,idx) in teaItemList" :key="idx" class="assessProblem">
          ...
          <div v-else class="InputScore">
            ...
            <input
              type="number"
              ref="inp"
              @input="inputTeaScore(item.ItemId,idx, item)"
              v-model="item.Result">
          </div>
        </div>
        <div class="assessComment" v-if="teaSysEvalItemData.IsComment === 1" style="margin-bottom: 60px;">
          <div v-if="!QD.stuAssessTeaIsWriteRelativeMerits">
            <p class="comment-title">对授课老师的意见或建议：</p>
            <x-textarea
              placeholder="请输入评语"
              :show-counter="false"
              :rows="1"
          ...
          <div v-else>
            <template>
              <p class="comment-title">优点：</p>
              <x-textarea
                placeholder="请输入任课老师优点"
                :show-counter="false"
                :rows="2"
                autosize
                v-model="assessRemarks">
            ...
            <template>
              <p class="comment-title">缺点：</p>
              <x-textarea
                placeholder="请输入任课老师缺点"
                :show-counter="false"
                :rows="2"
                autosize
                v-model="shortComing">
  ...
  export default {
    ...
    methods: {
      ...
      async getTeaSysEvalItemData() {
        ...
        this.teaItemList = d.ItemList; // 试题内容
        this.assessRemarks = this.teaSysEvalItemData.Comments; // 评语
        this.shortComing = this.teaSysEvalItemData.ShortComing; // 缺点
        for (let i = 0; i < this.teaItemList.length; i++){
          ...
          // 保存每题默认选中的作答的值
          if (this.isDefaultSelectFirstOP === "1") { // 判断是否默认选中第一项
      ...
      selectAssessVal (OptId, optId, val, idx, opidx) {
        ...
        console.log(OptId, optId, val, idx, opidx)
      ...
      async assessSubmit () {
        ...
        for (let i = 0; i < this.resultList.length; i++) {
          ...
          for (let j = 0; j < this.teaItemList.length; j++) {
            //遍历双重数组，形成提交数据。 如果答案与题目id相同，循环选项，如果选项的id值与题目的结果一致，则给满足条件的答案赋值
        ...
        this.loading = true;
        let d = await this.QD.getData('/ComApi/PostObject', {
          apiName: 'InsertSysEvalResultData',
          XH: this.QD.getLocalUserinfo().stuObj.XH,
        ...
        //console.log("InsertSysEvalResultData提交=》", d)
        this.loading = false;
        if (d === 1) {
          ...
          this.$router.replace({path: '/stu/studentAssessTea/studentAssessTeaLst'});
        } else {
          this.QD.alert('', '提交失败');
        }
      },
...
<style>
  .studentAssessKaoPing .xs-container {
    margin-top: 0;
  }
  .studentAssessKaoPing .stuAssessSubject{
    padding: 10px 15px;
    font-size: 16px;
    color: #333333;
    border-bottom: 1px solid #e5e5e5;
  }
  .studentAssessKaoPing .stuAssessPaper{
    font-size: 16px;
    color: #333333;
    line-height: 18px;
    padding:10px 4px;
  }
  .studentAssessKaoPing .assessSubjectContent .assessProblem{
    padding: 10px 12px;
    font-size: 14px;
    color: #666666;
  }
  .studentAssessKaoPing .assessSubjectContent .assessProblem span:nth-child(1){
    color: #333333;
  }
  .studentAssessKaoPing .assessSubjectContent .assessProblem span{
    padding-top: 2px;
    padding-bottom: 2px;
    margin-top: 2px;
    line-height: 18px;
  }
  .studentAssessKaoPing .assessSubjectContent .assessProblem .selectedBGColor{
    color: white;
    background-color: #44a9d4;
    border-radius: 3px;
  }
  .studentAssessKaoPing .assessSubjectContent .assessProblem input{
    width: 18px;
    height: 18px;
    margin-left: 4px;
    vertical-align: middle;
  }
  .studentAssessKaoPing .assessSubjectContent .assessProblem .InputScore{
    margin-top:10px;
  }
  .studentAssessKaoPing .assessSubjectContent .assessProblem .InputScore input{
    width:auto;
  }
  .studentAssessKaoPing .vux-x-textarea.weui-cell .weui-cell__bd textarea{
    font-size: 14px;
    color: #666666;
    margin-top:10px;
  }
  .studentAssessKaoPing .assessSubjectContent .assessSubmit{
    margin-bottom: 10px;
    text-align: center;
    background-color: #0f89e0;
    border-radius: 5px;
  }
  .studentAssessKaoPing .assessSubjectContent .assessSubmit a{
