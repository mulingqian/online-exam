    ...
    <div class="containers-common">
      ...
      <div v-if="r">
        <div class="detail">
          ...
          <div class="deSecond">
            <span class="bt"><span class="btword">标题</span><span class="ggbt">{{r.GGBT}}</span></span>
            <span class="bt"><span class="btword">发送范围</span>{{r.FSFW}}</span>
            <span class="bt"><span class="btword">接收部门</span>{{r.BMMC}}</span>
            <span class="bt"><span class="btword">显示置顶</span>{{rOnTop}}</span>
            <span class="bt"><span class="btword">截止时间</span>{{r.JZRQ}}</span>
            <span class="bt"><span class="btword">紧急度</span>{{r.JJD}}</span>
            <span class="bt"><span class="btword">重要性</span>{{r.ZYX}}</span>
            <span class="bt"><span class="btword">自动记录已阅</span>{{rYYJL}}</span>
            <span class="bt"><span class="btword">要求阅读回复</span>{{rYYQR}}</span>
  ...
  export default {
    ...
    methods: {
      ...
      goSelectMultiTeacher () {
          // 保存审批意见
        sessionStorage.setItem("noticePublish-verifyContent", this.verifyContent);
        // 之前的选择的教师信息
        if (this.selectMultiEmployeeUserIdLst !== '') {
          sessionStorage.setItem("selectMultiEmployeeNameLst_noticePublish", this.selectMultiEmployeeNameLst); // 存选择教师的姓名
          sessionStorage.setItem("selectMultiEmployeeUserIdLst_noticePublish", this.selectMultiEmployeeUserIdLst); // 存选择教师的UserId
        }
        this.$router.push({path: '/selectMultiTeacher', query: {page: 'noticePublishDetail', type: this.$route.query.type, id: this.$route.query.id, aId: this.$route.query.aId, rId: this.$route.query.rId}});
      },
      //  撤回操作type=0'新闻发布申请'列表（所有审批人尚未进行过审批操作时，提交人进行的撤回）RevokeBizData
      async revokeBizData () {
        let name = this.QD.getLocalUserinfo().stuObj.XM;
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.RevokeBizData', removerName: name, cellId: 'OA_FLOW_SQ_GGFB', bizId: this.bizId});
      ...
      async getCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_GGFB', auditedOnly: false});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      // 判断是从type=0'公告发布申请'列表，还是type=1'公告发布审批'列表的详情
      getType () {
        this.type = parseInt(this.$route.query.type);
        this.bizId = parseInt(this.$route.query.id);
        if (this.type === 0) {
          this.cellId = 'OA_FLOW_SQ_GGFB';
        } else {
          this.cellId = 'OA_FLOW_SP_GGFB';
        }
      },
      ...
      getSelectMultiTeacherData () {
        if (sessionStorage.getItem("selectMultiEmployeeNameLst_noticePublish") !== null && sessionStorage.getItem("selectMultiEmployeeUserIdLst_noticePublish") !== null){
          let name = sessionStorage.getItem("selectMultiEmployeeNameLst_noticePublish").split(",");
          let userId = sessionStorage.getItem("selectMultiEmployeeUserIdLst_noticePublish").split(",");
          ...
          sessionStorage.removeItem("selectMultiEmployeeNameLst_noticePublish");
          sessionStorage.removeItem("selectMultiEmployeeUserIdLst_noticePublish");
        } else {
          this.selectMultiEmployeeNameLst = ['选择'];
        }
        // 审批意见
        if (sessionStorage.getItem("noticePublish-verifyContent") !== null) {
          this.verifyContent = sessionStorage.getItem("noticePublish-verifyContent");
          sessionStorage.removeItem("noticePublish-verifyContent");
        }
      }
    },
    created () {
...
<style>
  .noticePublishDetail .verify .weui-cells__title{
    margin-top: 0;
    margin-bottom: 0;
    padding-top: 0.3em;
  ...
  .noticePublishDetail .verify .weui-cell_access .weui-cell__ft {
    display: inline-block;
    width: 60%;
    text-align: right;
    overflow: hidden;
