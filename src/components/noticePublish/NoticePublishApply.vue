    ...
    <group style="margin-bottom: 50px;">
      <cell>
        ...
        <span slot="default" class="rg-cell">
          <input type="text" class="noticeTitle" v-model="ggbt" placeholder="请输入标题" />
        </span>
      </cell>
      ...
      <popup-picker title="接收部门" v-model="receiptDepartment" :data="receiptDepartmentData" @on-change="onChangeReceiptDepartment"></popup-picker>
      <datetime
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 接收部门
      onChangeReceiptDepartment (val) {
        this.receiptDepartment = [val[0]];
      }
    },
    created () {},
    data () {
      return {
        ...
        fsfw: ['全校'], // 发送范围
        fsfwData: [['全校', '部门内部']],
        receiptDepartment: ['请选择'], // 接收部门
        receiptDepartmentData: [['保卫处', '财务处']],
