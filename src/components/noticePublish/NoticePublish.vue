<template>
  <div class="noticePublish">
    ...
    <tab>
      <tab-item :selected="showLst" @on-item-click="adjustApplyOrVerify(0)">公告申请</tab-item>
      <tab-item v-if="canAudit" :selected="!showLst" @on-item-click="adjustApplyOrVerify(1)">公告审批</tab-item>
    </tab>
    <!--公告发布申请-->
    <div v-show="showLst" class="applyShowLst">
      <ex-scroller ref="scrollerApplyPageIdxOne" v-model="NoticePublishApplyData" :onGetList="getNoticePublishApplyData" height="-90">
        <div>
          <group v-for="(n, index) in NoticePublishApplyData" :key="index">
            <cell is-link @click.native="goNoticePublishDetailPage(n.GGID,0,0)">
              <div slot="after-title" class="vux-label">
                ...
                <div class="firstLine">标题：{{n.GGBT}}</div>
    ...
    <div v-if="canAudit && !showLst" class="verifyShowLst">
      <ex-scroller ref="scrollerAuditPageIdxOne" v-model="noticePublishVerifyData" :onGetList="getNoticePublishVerifyData" height="-90">
        <div>
          <group v-for="(n, index) in noticePublishVerifyData" :key="index">
            <cell is-link @click.native="goNoticePublishDetailPage(n.GGID,n.AppId,n.REID)">
    ...
    <!--添加‘公告发布申请’-->
   <!-- <add-page @click.native="goNoticePublishApply()"></add-page>-->
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      // 判断是‘公告发布申请’列表，还是 ‘公告发布审核’列表
      adjustApplyOrVerify (type) {
        this.showType = type;
        this.showLst = this.showType === 0;
      },
      // 到‘公告发布详情’页面 showType=0代表'公告发布申请'列表，showType =1代表'公告发布审批'列表
      goNoticePublishDetailPage (id, aId, rid) {
        // 把'公告发布申请'的type 存sessionStorage,从'公告发布详情'页面 可以返回'公告发布申请'列表
        sessionStorage.setItem("noticePublishShowType", this.showType);
        this.$router.push({path: '/noticePublish/noticePublishDetail', query: {type: this.showType, id: id, aId: aId, rId: rid}});
      },
      // 到‘公告发布申请’页面
      goNoticePublishApply () {
        this.$router.push({path: '/noticePublish/noticePublishApply'});
      },
      ...
      // 获取‘公告发布申请’列表
      async getNoticePublishApplyData (index, size) {
        // 判断显示哪个列表
        if (sessionStorage.getItem("noticePublishShowType") !== null) {
          this.showType = parseInt(sessionStorage.getItem("noticePublishShowType"));
          this.showLst = this.showType === 0;
          // 清除存'公告发布申请'列表 noticePublishShowType
          sessionStorage.removeItem("noticePublishShowType");
        }
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetUserSubmitFlowData', cellId: 'OA_FLOW_SQ_GGFB', pageIndex: index, pageSize: size, conditionList: ''});
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
        console.log("notictPublishApplyData:", d.data);
        return d.data;
      },
      // 获取‘公告发布审核’列表
      async getNoticePublishVerifyData (index, size) {
        if (this.canAudit === true) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetUserAuditFlowData', cellId: 'OA_FLOW_SP_GGFB', pageIndex: index, pageSize: size, conditionList: ''});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
          console.log("noticePublishSPData:", d.data);
          return d.data;
        }
      },
      // 读取用户可申请和审批的流程菜单
      async getUserMenus () {
        ...
        for (let i=0; i<data.length; i++) {
          ...
          if (data[i].applyCellId === "OA_FLOW_SQ_GGFB"){
    ...
    data () {
      return {
        ...
        NoticePublishApplyData: [], // 公告发布申请列表
        noticePublishVerifyData: [], // 公告发布审核列表
...
<style>
  .noticePublish .verifyShowLst .weui-cells,
  .noticePublish .applyShowLst .weui-cells {
    position: relative;
    left: 2%;
    width: 96%;
    margin-top: 18px;
