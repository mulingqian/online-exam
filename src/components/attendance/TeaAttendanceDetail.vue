    ...
    <div class="containers-common">
      ...
      <div v-if="r">
        <div class="detail">
          ...
          <div class="deSecond">
            <span class="bt"><span class="btword">工号</span><span class="ggbt">{{r.ZGH}}</span></span>
            <span class="bt"><span class="btword">性别</span>{{r.XB}}</span>
            <span class="bt"><span class="btword">职务</span>{{r.JSZW}}</span>
            <span class="bt"><span class="btword">所在部门</span>{{r.BMMC}}</span>
            <span class="bt"><span class="btword">所在教研组</span>{{r.JYZMC}}</span>
            <span class="bt"><span class="btword">公事私事</span>{{leaveRange}}</span>
            <span class="bt"><span class="btword">请假类型</span>{{r.LeaveType}}</span>
            <span class="bt"><span class="btword">开始日期</span>{{r.KSRQ}}</span>
            <span class="bt"><span class="btword">结束日期</span>{{r.JSRQ}}</span>
            <span class="bt"><span class="btword">请假天数</span>{{r.Days}}</span>
            <span class="bt" v-if="r.SubmitTime !== ''"><span class="btword">提交时间</span>{{r.SubmitTime}}</span>
            <span class="bt"><span class="btword">请假事由</span>{{r.Reasons}}</span>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      goSelectMultiTeacher () {
        // 保存审批意见
        sessionStorage.setItem("teaAttendanceDetail-verifyContent", this.verifyContent);
        // 之前的选择的教师信息
        if (this.selectMultiEmployeeUserIdLst !== '') {
          sessionStorage.setItem("selectMultiEmployeeNameLst_teaAttendanceDetail", this.selectMultiEmployeeNameLst); // 存选择教师的姓名
          sessionStorage.setItem("selectMultiEmployeeUserIdLst_teaAttendanceDetail", this.selectMultiEmployeeUserIdLst); // 存选择教师的UserId
        }
        this.$router.push({path: '/selectMultiTeacher', query: {page: 'teaAttendanceDetail', type: this.$route.query.type, id: this.$route.query.id, aId: this.$route.query.aId, rId: this.$route.query.rId}});
      },
      // 审批‘通过’或 ‘拒绝’
      async doAudit (pass) {
        let appointUids = '';
      ...
      //  撤回操作type=0'职工请假申请'列表（所有审批人尚未进行过审批操作时，提交人进行的撤回）RevokeBizData
      async revokeBizData () {
        let name = this.QD.getLocalUserinfo().stuObj.XM;
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.RevokeBizData', removerName: name, cellId: 'OA_FLOW_SQ_KQSQ', bizId: this.bizId});
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
      ...
      async getGetFlowInfo () {
        ...
        this.r = d.data;
        this.leaveRange = this.r.LeaveRange === '1' ? '私事' : (this.r.LeaveRange === '2' ? '公事' : '其他'); // 私事=‘1’，公事=‘2’，其他=‘3’
        /*详情赋值*/
      ...
      async getCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_KQSQ', auditedOnly: false});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      // 判断是从type=0'考勤申请'列表，还是type=1'考勤申请审批'列表的详情
      getType () {
        this.type = parseInt(this.$route.query.type);
        this.bizId = parseInt(this.$route.query.id);
        if (this.type === 0) {
          this.cellId = 'OA_FLOW_SQ_KQSQ';
        } else {
          this.cellId = 'OA_FLOW_SP_KQSQ';
        }
      },
      // 根据流程申请业务模块的CellID和键值读取审批结果（审批步骤-审批人-回复内容等）auditedOnly = true仅查看已经审批的结果---用于判断是否显示‘撤回’
      async isGetCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_KQSQ', auditedOnly: true});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      getSelectMultiTeacherData () {
        if (sessionStorage.getItem("selectMultiEmployeeNameLst_teaAttendanceDetail") !== null && sessionStorage.getItem("selectMultiEmployeeUserIdLst_teaAttendanceDetail") !== null){
          let name = sessionStorage.getItem("selectMultiEmployeeNameLst_teaAttendanceDetail").split(",");
          let userId = sessionStorage.getItem("selectMultiEmployeeUserIdLst_teaAttendanceDetail").split(",");
          ...
          sessionStorage.removeItem("selectMultiEmployeeNameLst_teaAttendanceDetail");
          sessionStorage.removeItem("selectMultiEmployeeUserIdLst_teaAttendanceDetail");
        } else {
          this.selectMultiEmployeeNameLst = ['选择'];
        }
        // 审批意见
        if (sessionStorage.getItem("teaAttendanceDetail-verifyContent") !== null) {
          this.verifyContent = sessionStorage.getItem("teaAttendanceDetail-verifyContent");
          sessionStorage.removeItem("teaAttendanceDetail-verifyContent");
        }
      }
    },
    created () {
...
<style>
  .teaAttendanceDetail .verify .weui-cells__title{
    margin-top: 0;
    margin-bottom: 0;
    padding-top: 0.3em;
  ...
  .teaAttendanceDetail .verify .weui-cell_access .weui-cell__ft {
    display: inline-block;
    width: 60%;
    text-align: right;
    overflow: hidden;
