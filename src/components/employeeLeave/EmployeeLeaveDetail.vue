  ...
  export default {
    ...
    methods: {
      ...
      goSelectMultiTeacher () {
        // 保存审批意见
        sessionStorage.setItem("employeeLeaveDetail-verifyContent", this.verifyContent);
        // 之前的选择的教师信息
        if (this.selectMultiEmployeeUserIdLst !== '') {
          sessionStorage.setItem("selectMultiEmployeeNameLst_employeeLeave", this.selectMultiEmployeeNameLst); // 存选择教师的姓名
          sessionStorage.setItem("selectMultiEmployeeUserIdLst_employeeLeave", this.selectMultiEmployeeUserIdLst); // 存选择教师的UserId
        }
        this.$router.push({path: '/selectMultiTeacher', query: {page: 'employeeLeaveDetail', type: this.$route.query.type, id: this.$route.query.id, aId: this.$route.query.aId, rId: this.$route.query.rId}});
      },
      // 审批‘通过’或 ‘拒绝’
      async doAudit (pass) {
        let appointUids = '';
      ...
      async revokeBizData () {
        ...
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.RevokeBizData', removerName: name, cellId: 'OA_FLOW_SQ_ZGQJ', bizId: this.bizId});
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
      ...
      async getCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_ZGQJ', auditedOnly: false});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      // 判断是从type=0'职工请假申请'列表，还是type=1'职工请假审批'列表的详情
      getType () {
        this.type = parseInt(this.$route.query.type);
        this.bizId = parseInt(this.$route.query.id);
        if (this.type === 0) {
          this.cellId = 'OA_FLOW_SQ_ZGQJ';
        } else {
          this.cellId = 'OA_FLOW_SP_ZGQJ';
        }
      },
      // 根据流程申请业务模块的CellID和键值读取审批结果（审批步骤-审批人-回复内容等）auditedOnly = true仅查看已经审批的结果---用于判断是否显示‘撤回’
      async isGetCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_ZGQJ', auditedOnly: true});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      getSelectMultiTeacherData () {
        if (sessionStorage.getItem("selectMultiEmployeeNameLst_employeeLeave") !== null && sessionStorage.getItem("selectMultiEmployeeUserIdLst_employeeLeave") !== null){
          let name = sessionStorage.getItem("selectMultiEmployeeNameLst_employeeLeave").split(",");
          let userId = sessionStorage.getItem("selectMultiEmployeeUserIdLst_employeeLeave").split(",");
          ...
          sessionStorage.removeItem("selectMultiEmployeeNameLst_employeeLeave");
          sessionStorage.removeItem("selectMultiEmployeeUserIdLst_employeeLeave");
        } else {
          this.selectMultiEmployeeNameLst = ['选择'];
        }
        // 审批意见
        if (sessionStorage.getItem("employeeLeaveDetail-verifyContent") !== null) {
          this.verifyContent = sessionStorage.getItem("employeeLeaveDetail-verifyContent");
          sessionStorage.removeItem("employeeLeaveDetail-verifyContent");
        }
      }
    },
    created () {
...
<style>
  .employeeLeaveDetail .verify .weui-cells__title{
    margin-top: 0;
    margin-bottom: 0;
    padding-top: 0.3em;
  ...
  .employeeLeaveDetail .verify .weui-cell_access .weui-cell__ft {
    display: inline-block;
    width: 60%;
    text-align: right;
    overflow: hidden;
