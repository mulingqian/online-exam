      ...
      <div v-if="r">
        <div class="detail">
          ...
          <div class="deSecond">
            ...
            <span class="bt"><span class="btword">补休天数</span>{{r.Days}}</span>
            <span class="bt" v-if="r.SubmitTime !== ''"><span class="btword">提交时间</span>{{r.SubmitTime}}</span>
            <span class="bt"><span class="btword">补假事由</span>{{r.Reasons}}</span>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      goSelectMultiTeacher () {
        // 保存审批意见
        sessionStorage.setItem("deferHolidaysDetail-verifyContent", this.verifyContent);
        // 之前的选择的教师信息
        if (this.selectMultiEmployeeUserIdLst !== '') {
          sessionStorage.setItem("selectMultiEmployeeNameLst_deferHolidaysDetail", this.selectMultiEmployeeNameLst); // 存选择教师的姓名
          sessionStorage.setItem("selectMultiEmployeeUserIdLst_deferHolidaysDetail", this.selectMultiEmployeeUserIdLst); // 存选择教师的UserId
        }
        this.$router.push({path: '/selectMultiTeacher', query: {page: 'deferHolidaysDetail', type: this.$route.query.type, id: this.$route.query.id, aId: this.$route.query.aId, rId: this.$route.query.rId}});
      },
      // 审批‘通过’或 ‘拒绝’
      async doAudit (pass) {
        let appointUids = '';
      ...
      async revokeBizData () {
        ...
        let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.RevokeBizData', removerName: name, cellId: 'OA_FLOW_SQ_BXSQ', bizId: this.bizId});
        if (d.status && d.status === "error") {
          this.QD.alert(`获取数据失败:${d.msg}`);
        }
      ...
      async getCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_BXSQ', auditedOnly: false});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      // 判断是从type=0'补休申请'列表，还是type=1'补休申请审批'列表的详情
      getType () {
        this.type = parseInt(this.$route.query.type);
        this.bizId = parseInt(this.$route.query.id);
        if (this.type === 0) {
          this.cellId = 'OA_FLOW_SQ_BXSQ';
        } else {
          this.cellId = 'OA_FLOW_SP_BXSQ';
        }
      },
      // 根据流程申请业务模块的CellID和键值读取审批结果（审批步骤-审批人-回复内容等）auditedOnly = true仅查看已经审批的结果---用于判断是否显示‘撤回’
      async isGetCustomAuditResults () {
        if (this.type === 0) {
          let d = await this.QD.getData('/ComApi/PostObject', {apiName: 'apiOA.GetCustomAuditResults', bizId: this.$route.query.id, cellId: 'OA_FLOW_SQ_BXSQ', auditedOnly: true});
          if (d.status && d.status === "error") {
            this.QD.alert(`获取数据失败:${d.msg}`);
          }
      ...
      getSelectMultiTeacherData () {
        if (sessionStorage.getItem("selectMultiEmployeeNameLst_deferHolidaysDetail") !== null && sessionStorage.getItem("selectMultiEmployeeUserIdLst_deferHolidaysDetail") !== null){
          let name = sessionStorage.getItem("selectMultiEmployeeNameLst_deferHolidaysDetail").split(",");
          let userId = sessionStorage.getItem("selectMultiEmployeeUserIdLst_deferHolidaysDetail").split(",");
          ...
          sessionStorage.removeItem("selectMultiEmployeeNameLst_deferHolidaysDetail");
          sessionStorage.removeItem("selectMultiEmployeeUserIdLst_deferHolidaysDetail");
        } else {
          this.selectMultiEmployeeNameLst = ['选择'];
        }
        // 审批意见
        if (sessionStorage.getItem("deferHolidaysDetail-verifyContent") !== null) {
          this.verifyContent = sessionStorage.getItem("deferHolidaysDetail-verifyContent");
          sessionStorage.removeItem("deferHolidaysDetail-verifyContent");
        }
      }
    },
    created () {
...
<style>
  .deferHolidaysDetail .verify .weui-cells__title{
    margin-top: 0;
    margin-bottom: 0;
    padding-top: 0.3em;
  ...
  .deferHolidaysDetail .verify .weui-cell_access .weui-cell__ft {
    display: inline-block;
    width: 60%;
    text-align: right;
    overflow: hidden;
