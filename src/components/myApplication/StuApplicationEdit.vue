...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 判断可编辑菜单个数
      judgeCanEditNum () {
        if (this.myDefaultMenuLst.length >= this.QD.studentApplicationNum) {
          this.QD.alert('', `最多添加${this.QD.studentApplicationNum}个应用`);
          return false;
        }
      ...
      // 标记 ‘editAllMenuList’中所有存在于‘myDefaultMenuLst’的菜单
