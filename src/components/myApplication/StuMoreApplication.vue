      ...
      <div  class="igrid-field">
        <div class="igrid-box default-igrid-box">
          ...
          <div class="default-desc">
            <divider>以上应用展示在首页（最多{{QD.studentApplicationNum}}个）</divider>
          </div>
...
<script>
  ...
  export default {
    name: 'stuMoreApplication_keepAlive',
    directives: {
    },
    components: {
      Group,
    ...
    beforeRouteLeave(to, from, next) {
      ...
      // 记录滚动的高度
      if (to.name !== 'home') {
        let ct = document.querySelector('#menuContainer');
    ...
    computed: {
      ...mapState({
        ...
        scrollTop: state => state.vux.scrollTopAllMenu // ‘全部应用’滚动的高度
      })
    },
    methods: {
      ...mapActions([
      ...
      setPgScrollTop () {
        ...
        let content = document.querySelector('#menuContainer');
      ...
      // 保存本页面数据
      savePgData () {
        this.QD.application_currPageData = {allMenuList: this.allMenuList, myDefaultMenuLst: this.myDefaultMenuLst, newlyUseMenuLst: this.newlyUseMenuLst}; // 将‘全部应用’保存; 我的应用菜单保存
      },
      // 到我的应用编辑页面
      goMyAppEdit () {
        this.savePgData();
        this.$router.push({path: '/myApplication/stuApplicationEdit'});
      },
      // 展开显示 我的应用
      expandDefaultEvent () {
      ...
      saveNewlyUseMenuEvent () {
        let stuNewLyLst = {xh: this.userinfo.stuObj.XH, newLyLst: this.newlyUseMenuLst};
        let newlyUseMenuLst = JSON.stringify(stuNewLyLst);
        localStorage.setItem("stuNewlyUseLstData", newlyUseMenuLst);
      },
      // 初始化数据
      initDataWay () {
