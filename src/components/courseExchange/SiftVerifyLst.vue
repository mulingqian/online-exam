        ...
        <div style="padding:34px 0 55px 0;">
          <group class="gp">
            ...
            <div class="option-box">
              <a @click="selectStatusVal(s, sidx)" :class="selectIdx.statusIdx === sidx ? 'select-a-BgC' : ''" v-for="(s, sidx) in QD.auditStateData" :key="sidx">{{s.label}}</a>
            </div>
...
<script>
  ...
  export default {
    name: 'courseExchange-siftVerifyLst',
    directives: {
      TransferDom
    },
    components: {
    ...
    methods: {
      ...
      async getApplySemesters () {
        ...
        let d = await this.QD.getData('/ComApi/PostObject', {
          apiName: 'GetAllApplySemestersForTK'
        });
        this.loading = false;
